...
BuildingType* Map::isNextToBuilding(Tile* pTile, int distance) {
    ...
    for (int dy = -distance; dy <= distance; ++dy) {
        for (int dx = -distance; dx <= distance; ++dx) {
            ...
            // �berspringe das Zentrum (das gegebene pTile selbst)
            ...
            if (y >= 0 && y < m_arrTileMap.size() && x >= 0 && x < m_arrTileMap[0].size()) {
                ...
                // Versuche, die Einheit auf dem Tile als den gesuchten Geb�udetyp zu casten
                BuildingType* pBuilding = dynamic_cast<BuildingType*>(neighbourTile->getSeaOrLandUnit());

                // Wenn das Tile den gesuchten Geb�udetyp hat, gib diesen zur�ck
                if (pBuilding) {
                    return pBuilding;
                }
            }
    ...
    // Wenn kein passendes Geb�ude gefunden wurde, gib nullptr zur�ck
    return nullptr;
}
